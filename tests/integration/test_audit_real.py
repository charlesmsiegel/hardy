...
def _tools(declaration: str, project: Path) -> LeanTools:
    ...
    return LeanTools(
        request, (lake, 'env', 'lean'), timeout=120, project=project
    )


@pytest.mark.real_toolchain
def test_real_lean_reports_no_axioms_in_the_form_the_parser_expects(lean_project: Path) -> None:
...
@pytest.mark.real_toolchain
def test_real_lean_names_a_standard_axiom_the_proof_actually_used(lean_project: Path) -> None:
    ...
    tools = _tools('theorem HardyTarget (p : Prop) : p ∨ ¬p', lean_project)
...
@pytest.mark.real_toolchain
def test_real_lean_reports_sorry_ax_and_the_audit_names_it_a_hole(lean_project: Path) -> None:
    ...
    """
    tools = _tools('theorem HardyTarget : 1 = 2', lean_project)
...
@pytest.mark.real_toolchain
def test_a_primed_declaration_name_is_still_auditable(lean_project: Path) -> None:
    """The bug the shared parser fixed: `\\badd_comm'\\b` cannot match."""
    tools = _tools("theorem hardy_target' : 2 = 2", lean_project)
...
def _elaborator(project: Path):
    ...
    if lake is None:
        pytest.skip('lake is not installed')
    return lambda source: elaborate(source, argv=(lake, 'env', 'lean'), cwd=project, timeout_seconds=120)
...
@pytest.mark.real_toolchain
def test_a6_witness_is_accepted_or_refused_by_the_real_kernel(lean_project: Path) -> None:
    ...
    assert witness_verdict(_witness_entry('⟨1, by decide, trivial⟩'), elaborate=_elaborator(lean_project)) == 'witnessed'
    assert witness_verdict(_witness_entry('⟨0, by decide, trivial⟩'), elaborate=_elaborator(lean_project)) == 'broken'


@pytest.mark.real_toolchain
def test_a6_refuses_a_witness_that_is_a_hole(lean_project: Path) -> None:
