...
def test_the_summary_is_not_rebuilt_for_a_conversation_that_needs_no_compaction(tmp_path: Path) -> None:
    """`compact` runs before every provider call on the API backend, and
    assembling the summary scans the Lean tree, the audits and the whole
    ...
    calls: list[int] = []
    inner = chat._summary
    chat._summary = lambda: (calls.append(1), inner())[1]
