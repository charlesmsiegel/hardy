...
- `Interfaces.lean` replaces the packet's `constant`s with `PlaneGeometry F`
  ...
  monodromy, ...) as uninterpreted data a theorem takes. Connectedness of
  `S_F` and membership of a common pencil are definitions over the record
  (connected components are the classes generated by meeting; a pencil is the
  affine class of a defining polynomial), and one coherence field,
  `disjoint_of_pencilOf_eq`, says that distinct members of one pencil are
  disjoint. A statement over the record has the claim's exact shape; it
  becomes a statement about varieties only when the fields are given meaning.
...
- `keller-groupoids-rank-two/lean/KellerPlanar/`: thirty-seven theorems.
  Eighteen are kernel-verified (the permutation-group and arithmetic cores,
  the connectivity lemmas, `KG-PM02`, `KG-PM03`, `KG-PC02`, `KG-E02`); six are
  proved modulo named literature or external axioms and wait on their
  admission (`KG-P12`, `KG-A600`, the deficit arithmetic of `KG-A601`,
  `KG-EM08` from Jelonek's parametrisation and Assi's trichotomy, `KG-EM09`
  from Assi's dichotomy and Braun–Dias–Venato-Santos, and `KG-EM10`, which is
  derived from `KG-EM07`–`KG-EM09` exactly as the audit derives it and stays
  open through `KG-EM07`); thirteen are typed statements ending in `sorry`,
  the open holes of the étale-maximality chain, perfect monodromy and the
  `A_6` packet.
