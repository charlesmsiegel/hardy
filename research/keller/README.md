...
human check. Eight items are `lean verified`: a Lean statement of the whole
...
| `keller-groupoids-rank-two` | 5 | 30 | 15 | 11 | 24 published, 2 external research | 0 |

`tools/check.py` prints the current board. The eight kernel-verified claims:
...
cubic); `KG-PC01` (the fiber-deficit decomposition `d - m_C = |supp σ_C| + t_C`)
and `KG-PC02` (deficit one forces one deleted sheet, now from `KG-PC01` itself),
...
A claim is promoted only when every hypothesis of its Lean statement is a
literature axiom, a coherence field of the interface record, or a theorem
already proved in the tree. A declaration that assumes another project claim,
or a geometric input this tree does not formalise, is recorded as a `core` of
its claim and leaves the status alone, however complete its own proof is. The
étale-maximality chain, perfect monodromy and the `A_6` packet are all of that
kind, and each note says which prerequisite was assumed.
...
- `Interfaces.lean` replaces the packet's `constant`s with `PlaneGeometry F`
  ...
  monodromy, ...) as uninterpreted data a theorem takes. The incidence of
  components and points is primitive: `branchCount C p` is the number of
  normalisation branches of the component `C` over the point `p`. Meeting,
  connectedness, membership of a common pencil, unibranch and multibranch
  singularities are then definitions over the record, and two coherence fields
  say that distinct members of one pencil share no point and that every listed
  point lies on the curve. A statement over the record has the claim's exact
  shape; it becomes a statement about varieties only when the fields are given
  meaning.
...
- `keller-groupoids-rank-two/lean/KellerPlanar/`: fifty-two theorems.
  Thirty-seven are kernel-verified on standard axioms alone: the
  permutation-group and arithmetic cores, the incidence-graph and
  branch-counting lemmas behind `KG-EM07`, the deficit decomposition
  `KG-PC01` and its corollary `KG-PC02`, the unibranch criterion `KG-EM04`,
  the perfectness of a group generated by ratios of conjugate elements
  (`KG-PM01`), the forced `A_6` node inertia (`KG-A603`), `KG-PM02`, `KG-PM03`
  and `KG-E02`. Nine are proved modulo named literature or external axioms and
  wait on their admission: `KG-P12`, `KG-A600`, the `A_6` boundary packet
  `KG-A601`, `KG-EM07` from Jelonek's Euler positivity, `KG-EM08` from
  Jelonek's parametrisation and Assi's trichotomy, `KG-EM09` from Assi's
  dichotomy and Braun–Dias–Venato-Santos, and `KG-EM10`, the two-nodal-fiber
  theorem, derived from `KG-EM07`–`KG-EM09` exactly as the audit derives it.
  Five are typed statements ending in `sorry`: `KG-PC03` and the boundary
  theorems `KG-EM01`, `KG-EM02`, `KG-EM03`, `KG-EM05`.

  Those five are the tree's real remaining holes. Each needs notions the
  record does not carry and Mathlib does not have for varieties over `ℂ`:
  purity of the branch locus and Zariski's Main Theorem for `KG-EM01` and
  `KG-EM02`, the immersivity of a normalisation at a unibranch singularity
  for `KG-EM03`, and the local fiber over a conductor point for `KG-PC03` and
  `KG-EM05`. Proving them over the record would mean assuming their own
  conclusions, so they stay `sorry` rather than being dressed up.
