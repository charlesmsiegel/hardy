...
- `keller-groupoids-rank-two/lean/KellerPlanar/`: forty-six theorems.
  Twenty-seven are kernel-verified (the permutation-group and arithmetic
  cores, the incidence-graph and branch-counting lemmas behind `KG-EM07`,
  `KG-PM02`, `KG-PM03`, `KG-PC02`, `KG-E02`); eight are proved modulo named
  literature or external axioms and wait on their admission (`KG-P12`,
  `KG-A600`, the deficit arithmetic of `KG-A601`, `KG-EM07` from Jelonek's
  Euler positivity, `KG-EM08` from Jelonek's parametrisation and Assi's
  trichotomy, `KG-EM09` from Assi's dichotomy and Braun–Dias–Venato-Santos,
  and `KG-EM10`, the two-nodal-fiber theorem, derived from `KG-EM07`–`KG-EM09`
  exactly as the audit derives it and now resting on no hole); eleven are
  typed statements ending in `sorry`, the remaining open holes of the
  étale-maximality chain, perfect monodromy and the `A_6` packet.

  The `KG-EM07` chain is the one place where a claim's prerequisites are
  explicit hypotheses rather than Lean proofs: the Euler count
  `χ(S_F) = c - Σ_p (r_p - 1)` off the `A¹` normalisations, and the
  multibranch consequence of non-maximality. Each is named in the theorem's
  docstring and in `tools/lean-map.json`.
