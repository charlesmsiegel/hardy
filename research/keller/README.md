...
- `Interfaces.lean` replaces the packet's `constant`s with `PlaneGeometry F`
  ...
  monodromy, ...) as uninterpreted data a theorem takes, plus one coherence
  field, `selectedFibers_ge_two`, saying that a disconnected `S_F` in a common
  pencil selects at least two fibers. A statement over the record has the
  claim's exact shape; it becomes a statement about varieties only when the
  fields are given meaning.
...
- `keller-groupoids-rank-two/lean/KellerPlanar/`: thirty-four theorems.
  ...
  `KG-PM02`, `KG-PM03`, `KG-PC02`, `KG-E02`); five are proved modulo named
  literature or external axioms and wait on their admission (`KG-P12`,
  `KG-A600`, the deficit arithmetic of `KG-A601`, `KG-EM09` from Assi's
  dichotomy and Braun–Dias–Venato-Santos, and `KG-EM10`, which is derived from
  `KG-EM07`–`KG-EM09` exactly as the audit derives it and stays open through
  the first two); fourteen are typed statements ending in `sorry`, the open
  holes of the étale-maximality chain, perfect monodromy and the `A_6` packet.
