...
# Pinned by identity, not by "whatever Mathlib's master says today": the Lean
...
# The project files themselves, written rather than generated by `lake init`:
# the `math` template requires Mathlib at whatever its default branch holds,
# and a project that moves when upstream does cannot be the environment a
# recorded result names. `lean-toolchain` is what elan reads to pick the
# compiler, and the manifest `lake update` writes from this lakefile is what
# `hardy` records as the Mathlib revision.
write_lean_project() {
	printf '%s\n' "$LEAN_TOOLCHAIN" >"$LEAN_PROJECT/lean-toolchain" || return 1
	cat >"$LEAN_PROJECT/lakefile.toml" <<EOF || return 1
name = "$LEAN_PACKAGE"
...
name = "HardyMath"
EOF
	printf 'import Mathlib\n' >"$LEAN_PROJECT/HardyMath.lean" || return 1
}
...
ensure_lean_project() {
	...
	if [ ! -e "$LEAN_PROJECT/lakefile.toml" ] && [ ! -e "$LEAN_PROJECT/lakefile.lean" ]; then
		...
		say "creating a Lake project pinned to $LEAN_TOOLCHAIN and Mathlib $MATHLIB_REVISION"
		write_lean_project || fail "writing the pinned Lake project under $LEAN_PROJECT failed"
	else
