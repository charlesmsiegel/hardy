...
- **Now (implemented) — `sorry`-backed sketches, unattended:** `hardy prove` and
  ...
  actually opens while the Lean beneath it stays honest.
...
- **Now (implemented):** cheap Lean closers before a model turn is spent —
  ...
  events: a forged `closed_by`, a removed attempt, a missing decline, or a
  recorded provider exchange beside a ladder that claims to have closed the
  statement is refused. `closed_by` names a tactic only when the run *kept* its
