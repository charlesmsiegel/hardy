...
- **Now (implemented):** preserve the original statement and reject completed artifacts that use
  ...
  kept on the two surfaces with somewhere to keep it — `sketch_proof` in `hardy
  batch`, an ordinary save interactively. The staged `hardy prove` path has
  neither. See the two sketch entries below.
...
- **Now (implemented) — `sorry`-backed sketches, unattended:** `hardy batch`
  ...
  with each other and with the event Lean produced. A submission is the third
  door a kept candidate comes through: a body Lean accepted and the axiom
  report then refused replaces the skeleton it supersedes, since the holes were
  the run's remaining work two events ago and the axiom is its remaining work
  now. The record says which door, and the writeup says the true thing for
  each — a candidate the audit refused is not reported as one nothing has
  audited, and the flag that chooses between those two sentences is re-derived
  from the events rather than believed. The event carries Lean's own answer
  (`lean_accepted`) beside the audit's, because the audit rewrites a
  submission's `ok` and a body Lean refused would otherwise be indistinguishable
  from one it accepted. The writeup's
  section is generated by one function and required verbatim, so a partial
  result cannot have "1 hole" edited to "0 holes" in the artifact a reader
  actually opens while the Lean beneath it stays honest. The kept sketch is also
  required to carry, and to match, the source Lean was actually given, rebuilt from the
  request the way `proof.lean` is — every other comparison is the record
  against itself, and the tool result's source and its hash are the one thing
  in the trajectory that came out of Lean. The hole list is likewise
  recomputed from the skeleton itself: edited consistently across all three
  copies and the event they came from, the copies agree with each other and
  conceal the hole from every one of them, and Lean's own rule is the only
...
- **Now (implemented):** cheap Lean closers before a model turn is spent —
  ...
  submission — a check that began inside the deadline and finished outside it
...
- **Now (implemented):** an interactive session accumulates the cost and token
  ...
  covering part of a session says which part. On the harness-owned loop an
  exchange publishes the running total for the counters it actually stated and
  no others, so a figure reported once early is never restated as though a
  later exchange had measured it too. A terminal too narrow for the
...
- **Now (implemented):** compaction Hardy owns, with a summary derived from the
  ...
  recorded `after` is the whole of what will be sent — the system prompt and
  tool schemas the provider charges for whatever the conversation holds
  included, which on a workspace whose `AGENTS.md` is in the prompt is a
  substantial part of the window — and `fits` compares it against the window
  rather than against the conversation it replaced — an
