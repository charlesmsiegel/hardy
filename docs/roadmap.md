...
**Status:** `hardy web` implemented (2026-09-14): a loopback client running the terminal's own registry over one live session per problem, with chats as separate transcripts under `chats/<id>/`, and uploads staged under `.local/uploads/` and promoted through `/import` or the library. Second pass (2026-09-15, [design](superpowers/specs/2026-09-15-workbench-second-pass-design.md)): every computer algebra cell is a file under `cas/`; a model picker; slash-command completion in the composer; a message sent during a turn is queued; long Lean, LaTeX and CAS calls detach into background jobs under the delegation hierarchy and their results reach the model at its next turn, with Hardy starting a turn of its own when the session is idle; `/checkpoint` saves and restores the whole workspace. Not done: concurrent chats, portable collaborator metadata.

The page itself was rebuilt as "the workbench," a five-shipment plan. Shipment 1 (2026-09-16) replaced the old three-column client (`App.jsx`, `Sidebar.jsx`, `Panels.jsx`, `panels/*.jsx`, all deleted) with hash-based routing and a strict component vocabulary, and landed nine pages sourced directly from the session: Home, Chat, Chat › Tree, Jobs, Library, Files (read-only: trees, viewer, uploads, the `cas/` cell journal drawn as cells), Ledger (provenance graph only), Environment and Help. Three new read-only endpoints back it -- `GET /api/environment`, `GET /api/record`, `GET /api/chats` -- and `GET /api/graph` now carries a `family` token per node, a `style` token per edge, and a `tones` map over every exact `ObligationStatus`, rather than collapsing six values into three buckets. Five tabs were visible in the workbench (Results, Runs, Publications, Checkpoints, Status) with no page behind them yet, rendering an honest "not built yet" state. Shipment 2 (2026-09-16) landed the record surfaces: eight new read-only endpoints (`GET /api/ledger`, `GET /api/ledger/item`, `GET /api/ledger/export`, `GET /api/results`, `GET /api/runs`, `GET /api/runs/item`, `GET /api/publications`, `GET /api/checkpoints`) and the five pages they back -- Results (the three-lane theorem view), the Ledger's list and item views alongside its existing provenance graph (with an Export proof card in the item pane), Runs, Publications and Checkpoints -- plus a correction to the Ledger graph's own stale-version sentence. Shipment 3 (2026-09-17) made the client able to write. `PUT /api/file` saves an edited Lean or TeX file through `MathematicsSession.save_authored` -- the same save the model's own `save_lean`/`save_latex` tools reach, so the check, the shadow build of every importer, the result and documentation gates and the axiom audit all apply, and the only thing skipped is the model-loop save-streak brake, whose sentence is addressed to a caller in a loop and which is cleared per model turn. `POST /api/check` runs the same check and saves nothing. `GET /api/declarations` and `GET /api/declaration` serve the existing `DeclarationIndex`, read in the background and reported as unread rather than blocking a tab on a cold Mathlib scan. `GET /api/files` rows gained size, mtime, declarations and the file's own kernel verdict. Shipment 4 (2026-09-17) landed Status and `GET /api/spend`: two of its four spend scopes carry figures and two state that nothing measures them, because spend is kept as one session-wide total that nothing partitions by conversation branch. With Status built, no tab renders a "not built yet" state and that fallback is gone. Shipment 5 is the remaining cross-page sweep. Issue #174 (2026-09-18) gave `GET /api/runs/item` the frozen statement itself: `claim` is read from the run's own `formalization.json` and served beside `claim_sha256` only when it re-freezes to that hash, and a run whose manifest names a hash its directory no longer carries says so in `claim_error` rather than the page implying the text is never served.
...
**Status:** Backend complete (2026-09-10), all fourteen slices: delegation contracts and the hash-chained journal; ancestor-bounded leases with unknown liability and recursive cancellation; a local worker executor and the leaf worker with its own provider context; the problem core, research brief and staged initial working set with a diversity portfolio; lazy retrieval with isolation inherited down the tree; structured findings with upward, lateral and discoverable promotion; the mechanical lane-and-tranche scheduler; the optional bounded coordinator with explicit authority; per-worker workspace overlays and change sets; subtree project overlays with local admission; current-head reconciliation and crash-safe authoritative admission; routed attention with journaled subscriptions, safe-boundary interrupts and epoch-guarded continuations; the `/delegate` and `/jobs` control surface with a `delegation_workers` setting; and a hermetic end-to-end acceptance suite (one session worker admitted to the ledger; an eight-worker tree with a blind cell, an adversary, an assisted coordinator, conflicting change sets, duplicate findings, root exhaustion and a cancelled cell). Not built: distributed executors, book ingestion, a rich swarm UI, and application-installed capability owners, so authoritative admission from the terminal waits on the readers the ledger policy already requires. Per-worker child-process interruption is cooperative (runtime cancel plus tokens and per-call Lean timeouts), since process interruption is process-wide.
