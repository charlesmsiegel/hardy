...
docker/Dockerfile                   — sandbox image: toolchain + Mathlib cache + repl + tectonic
...
- `docker` — needs the `hardy-sandbox:dev` image built.
...
markers = [
    ...
    "tex: needs tectonic on PATH",
    "docker: needs the hardy-sandbox:dev Docker image",
]
```
...
- Produces: `hardy.lean.repl` — exceptions `LeanReplError`, `ReplTimeout(LeanReplError)`, `ReplDied(LeanReplError)`; class `LeanRepl(argv: list[str], cwd: Path | None = None, default_timeout: float = 60.0)` with `async start()`, `async send(request: dict, timeout: float | None = None) -> dict`, `async run_command(code: str, env: int | None = None, timeout: float | None = None) -> CommandResponse`, `async run_tactic(tactic: str, proof_state: int, timeout: float | None = None) -> TacticResponse`, `async close()`, properties `alive: bool` and `pid: int | None`. **A `ReplTimeout` or `ReplDied` leaves the process killed (`alive == False`)** — callers must treat the worker as dirty and replace it. One in-flight request per instance (the pool guarantees this).
...
On timeout or process death the process is killed and left dead: a worker
that hung or crashed mid-command has unknowable state and must be replaced,
never reused (DESIGN.md Component 1, environment isolation).
"""
...
- Produces: `hardy.lean.pool.ReplPool(*, size: int, argv: list[str], cwd: Path | None = None, imports: str = "import Mathlib", import_timeout: float = 600.0, command_timeout: float = 60.0, max_commands: int = 500, max_rss_mb: int = 12_000)` with `async start()`, `async check_proof(code: str, timeout: float | None = None) -> ProofVerdict`, `async close()`. Every check runs with `env=base_env` (the environment id returned by the imports command) — declarations from one check are invisible to the next. Timeout/crash returns `failure_verdict(...)` and replaces the worker. Task 8's benchmark calls exactly this API.
...
once at spawn and records the returned environment id as base_env. Every
check_proof forks from base_env, so declarations, axioms, and instances
introduced by one run are invisible to every other run — warm process,
clean declarations. Workers are replaced (never reused) after a timeout,
crash, or protocol error, after max_commands checks, or when resident
memory exceeds max_rss_mb.

Memory caveat: the RSS check reads the spawned process via psutil, which is
meaningful for directly-spawned workers. When argv is a `docker run -i`
wrapper the local process is just the docker client — there the container's
own --memory limit (hardy.sandbox) is the real cap.
"""
...
class PoolWorker:
    def __init__(self, repl: LeanRepl, base_env: int):
        self.repl = repl
...
class ReplPool:
    ...
    async def start(self) -> None:
        workers = await asyncio.gather(*(self._spawn() for _ in range(self._size)))
        for worker in workers:
            self._idle.put_nowait(worker)

    async def _spawn(self) -> PoolWorker:
        repl = LeanRepl(self._argv, cwd=self._cwd, default_timeout=self._command_timeout)
        await repl.start()
        resp = await repl.run_command(self._imports, timeout=self._import_timeout)
        if resp.env is None:
            await repl.close()
            raise LeanReplError(f"worker imports failed: {resp}")
        return PoolWorker(repl, base_env=resp.env)
...
- Produces:
  ...
  - `hardy.latex.compile` — `TexError(file: str | None, line: int | None, message: str)`; `CompileResult(success: bool, errors: list[TexError], log_tail: str, pdf_path: Path | None)`; `DEFAULT_ENGINE = ["tectonic", "--untrusted", "--chatter", "minimal"]`; `compile_tex(source: str, staging: Path, *, engine: list[str] | None = None, extra_env: dict[str, str] | None = None, timeout: float = 120.0) -> CompileResult`. The engine argv gets `"main.tex"` appended — so a docker-prefixed argv (Task 7) slots straight in. The subprocess environment is **scrubbed**: only `PATH`, `HOME=<staging>`, plus `extra_env`.
...
sees only the staging directory, and the subprocess environment is scrubbed
to PATH + HOME (+ explicit extra_env) so nothing can leak through it.

The engine argv gets "main.tex" appended, so a sandbox wrapper argv
(docker run ... tectonic --untrusted --only-cached ...) slots in unchanged.
"""
...
def _parse_errors(stderr: str, log_path: Path) -> list[TexError]:
    errors = []
    ...
    *,
    ...
    try:
        ...
        return CompileResult(
            success=False,
            errors=[TexError(message=f"compile timed out after {timeout}s")],
        )
    errors = _parse_errors(proc.stderr, staging / "main.log")
    pdf = staging / "main.pdf"
    success = proc.returncode == 0 and pdf.exists()
    if not success and not errors:
        errors = [TexError(message=(proc.stderr.strip() or "compile failed")[-500:])]
    return CompileResult(
        success=success,
...
- Modify: `src/hardy/lean/launch.py` (add `sandboxed_repl_argv`)
...
- Produces:
  - `hardy.sandbox.runner` — `Mount(host: str, container: str, mode: Literal["ro", "rw"] = "ro")`; `SandboxConfig(image: str, tmpfs_path: str = "/scratch", tmpfs_size_mb: int = 512, tmpfs_inodes: int = 10_000, pids_limit: int = 256, memory_mb: int = 8192, cpus: float = 2.0, env: dict[str, str] = {"TMPDIR": "/scratch"}, mounts: list[Mount] = [], workdir: str | None = None)`; `docker_argv(cfg: SandboxConfig, command: list[str], *, interactive: bool = False) -> list[str]`. **No network, read-only rootfs, cap-drop, no-new-privileges are hardcoded, not config.**
  - `hardy.lean.launch.sandboxed_repl_argv(image: str = "hardy-sandbox:dev", memory_mb: int = 12_288) -> list[str]` — pool-ready argv (use with `cwd=None`).
  - The `hardy-sandbox:dev` Docker image: pinned toolchain, built Mathlib + repl, tectonic with a warmed package cache, and `/home/hardy/repl-env.sh` exporting a build-time-captured `LEAN_PATH` so nothing invokes `lake` at runtime (the rootfs is read-only).
...
def cfg(**kwargs) -> SandboxConfig:
    kwargs.setdefault("image", "hardy-sandbox:dev")
...
def test_mounts_workdir_and_interactive():
    ...
    assert argv[-4:] == ["hardy-sandbox:dev", "tectonic", "--untrusted", "main.tex"]
...
RUN apt-get update && apt-get install -y --no-install-recommends \
        curl git ca-certificates fontconfig \
...
Build (after `scripts/setup_lean.sh` has populated `vendor/repl`):

```bash
docker build -f docker/Dockerfile -t hardy-sandbox:dev .
```

Expected: image builds; the `diff` line fails the build loudly if the repl checkout drifted from the pinned toolchain.
...
```python
def sandboxed_repl_argv(
    image: str = "hardy-sandbox:dev", memory_mb: int = 12_288
) -> list[str]:
    """Pool-ready argv running the repl inside the sandbox (use cwd=None).
    ...
    cfg = SandboxConfig(image=image, memory_mb=memory_mb)
    command = [
        "/bin/sh",
        "-c",
        ". /home/hardy/repl-env.sh && exec /home/hardy/repl/.lake/build/bin/repl",
    ]
    return docker_argv(cfg, command, interactive=True)
```
...
```python
"""Integration tests against the built sandbox image.

Prereq: docker build -f docker/Dockerfile -t hardy-sandbox:dev .
...
CFG = SandboxConfig(image="hardy-sandbox:dev")
...
def test_pids_limit_applied():
    ...
    proc = subprocess.run(
        docker_argv(
            SandboxConfig(image="hardy-sandbox:dev", pids_limit=16),
            ["/bin/sh", "-c",
...
async def test_sandboxed_repl_checks_a_proof():
    from hardy.lean.launch import sandboxed_repl_argv
    from hardy.lean.repl import LeanRepl

    repl = LeanRepl(sandboxed_repl_argv(), default_timeout=600)
    await repl.start()
    try:
        resp = await repl.run_command("theorem t : 2 + 2 = 4 := rfl")
...
def test_sandboxed_tectonic_compiles_offline(tmp_path):
    from hardy.latex.compile import compile_tex
    from hardy.latex.template import render_writeup
    from hardy.sandbox.runner import Mount

    staging = tmp_path / "staging"
    staging.mkdir()
    cfg = SandboxConfig(
        image="hardy-sandbox:dev",
        mounts=[Mount(host=str(staging), container="/work", mode="rw")],
        workdir="/work",
    )
    engine = docker_argv(
        cfg, ["tectonic", "--untrusted", "--only-cached", "--chatter", "minimal"]
    )
    source = render_writeup(
        title="Sandbox Check",
        statement=r"$1 + 1 = 2$.",
...
Expected: 5 PASS (first sandboxed-repl response is slow — Mathlib import).
...
git commit -m "feat: sandbox layer (no-network read-only containers) + hardy-sandbox image"
```
...
**Interfaces:**
- Consumes: `ReplPool.check_proof` (Task 5); `repl_argv`, `sandboxed_repl_argv`, `LEAN_PROJECT` (Tasks 2/7); `render_writeup` (Task 6); `compile_tex`, `CompileResult` (Task 6); `SandboxConfig`, `Mount`, `docker_argv` (Task 7).
...
from hardy.lean.launch import LEAN_PROJECT, repl_argv, sandboxed_repl_argv
from hardy.lean.pool import ReplPool
...
async def main() -> int:
    ...
    parser.add_argument("--image", default="hardy-sandbox:dev")
    args = parser.parse_args()

    if args.sandbox:
        argv, cwd = sandboxed_repl_argv(args.image), None
    else:
        argv, cwd = repl_argv(), LEAN_PROJECT

    pool = ReplPool(size=args.workers, argv=argv, cwd=cwd)
...
def main() -> int:
    ...
    if args.sandbox:
        cfg = SandboxConfig(
            image=args.image,
            mounts=[Mount(host=str(staging.resolve()), container="/work", mode="rw")],
            workdir="/work",
        )
        engine = docker_argv(
            cfg, ["tectonic", "--untrusted", "--only-cached", "--chatter", "minimal"]
...
In `README.md`, change the Status section:
...
- **Order matters only where interfaces say so**: Task 2 (Lean build) is slow (Mathlib cache download) — kick it off first; Tasks 1/4/6 are pure Python and can proceed while it downloads. Task 3 needs Tasks 1–2; Task 5 needs 3–4; Task 7 needs 2 (for `vendor/repl`) and 6 (only for its compile integration test); Task 8 needs everything.
