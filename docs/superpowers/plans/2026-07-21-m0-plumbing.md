...
- Produces: `hardy.lean.pool` —
  ...
  - `ReplPool(*, size: int, argv: list[str] | None = None, cwd: Path | None = None, spec_factory: Callable[[], WorkerSpec] | None = None, imports: str = "import Mathlib", import_timeout: float = 600.0, command_timeout: float = 60.0, max_commands: int = 500, max_rss_mb: int = 12_000)` with `async start()`, `async check_proof(code: str, timeout: float | None = None) -> ProofVerdict`, `async close()`. Pass `argv`/`cwd` for identical direct workers, or `spec_factory` (called once per worker spawn) when each worker needs its own spec — e.g. unique sandbox container names. Every check runs with `env=base_env` (the environment id returned by the imports command, admitted only if the import verdict is `complete`) — declarations from one check are invisible to the next. Timeout/crash returns `failure_verdict(...)` and replaces the worker; a failed `reset_argv` also replaces it. A failed `start()` closes any workers that did spawn before re-raising. Task 8's benchmark calls exactly this API.
...
reset_argv wipes the container's /scratch between checks (resetting the
Lean environment id does not reset filesystem state — without the wipe,
one untrusted proof could leave files for, or exhaust the quota of, the
...
class ReplPool:
    def __init__(
        ...
        *,
        ...
        self._idle: asyncio.Queue[PoolWorker] = asyncio.Queue()
...
- Produces:
  ...
  - `hardy.latex.compile` — `TexError(file: str | None, line: int | None, message: str)`; `CompileResult(success: bool, errors: list[TexError], log_tail: str, pdf_path: Path | None)`; `DEFAULT_ENGINE = ["tectonic", "--untrusted", "--chatter", "minimal"]`; `compile_tex(source: str, staging: Path, *, engine: list[str] | None = None, extra_env: dict[str, str] | None = None, timeout: float = 120.0) -> CompileResult`; `sandboxed_tex_engine(staging: Path, image: str = "hardy-tex:dev") -> list[str]` (lazy-imports `hardy.sandbox.runner` from Task 7 — unit tests here never touch it). The engine argv gets `"main.tex"` appended — so a docker-prefixed argv slots straight in. Containment: subprocess environment **scrubbed** to `PATH`, `HOME=<staging>`, plus `extra_env`; combined output hard-capped at 1 MB (over-cap kills the compile and reports it); `main.log` read tail-only; stale `main.pdf`/`main.log` deleted before every run. The sandboxed engine compiles in the quota'd `/scratch` tmpfs with the source mounted read-only and copies back only `main.pdf`/`main.log`.
...
def sandboxed_tex_engine(staging: Path, image: str = "hardy-tex:dev") -> list[str]:
    """Engine argv running tectonic inside the sandbox (DESIGN.md Comp. 5/7).
    ...
    cannot fill host storage — under an in-container `timeout` (so a hung
    compile ends even if the docker client is killed first); afterwards the
    trusted tail of the script copies back only main.pdf and main.log,
    each bounded by the scratch quota. The image (hardy-tex:dev) contains
    no Lean project or other repo state, so TeX \\input primitives have
    nothing to disclose beyond the staging dir itself. compile_tex appends
    "main.tex" to this argv — it lands in sh's unused $0 slot.
    """
    ...
    script = (
        ...
        "timeout 240 tectonic --untrusted --only-cached --chatter minimal main.tex; "
...
- Produces:
  ...
  - `hardy.lean.launch.sandboxed_worker_spec(image: str = "hardy-lean:dev", memory_mb: int = 12_288) -> WorkerSpec` — pool-ready spec (pass as `spec_factory`); each call mints a unique container name and wires `reset_argv` (per-check `/scratch` wipe) and `cleanup_argv` (`docker kill`).
...
def sandboxed_worker_spec(
    ...
    side-channels can address the container itself: reset_argv wipes
    /scratch between checks (one untrusted proof must not leave files for
    the next), and cleanup_argv kills the container — killing only the
    docker client would strand a hung REPL, since SIGKILL is never proxied
    and --rm removes a container only after it exits on its own.
    ...
    return WorkerSpec(
        ...
        reset_argv=[
            "docker", "exec", name, "find", "/scratch", "-mindepth", "1", "-delete",
        ],
...
def test_pids_limit_applied():
    # A shell swallows failed background forks and can still exit 0, so assert
    # on the live process count inside the container instead of the exit code.
    proc = subprocess.run(
        docker_argv(
            SandboxConfig(image="hardy-lean:dev", pids_limit=16),
            ["/bin/sh", "-c",
             "for i in $(seq 1 64); do sleep 2 & done; ls /proc | grep -c '^[0-9]'"],
        ),
        capture_output=True, text=True, timeout=60,
    )
    live = int(proc.stdout.strip().splitlines()[-1])
    assert live <= 20  # forks beyond the 16-pid ceiling must have failed
...
def test_sandboxed_tectonic_compiles_offline(tmp_path):
    ...
    result = compile_tex(
        source, staging, engine=sandboxed_tex_engine(staging), timeout=300
    )
    assert result.success, result.errors
...
def main() -> int:
    ...
    engine = (
        sandboxed_tex_engine(staging, image=args.image) if args.sandbox else None
    )
    result = compile_tex(source, staging, engine=engine, timeout=300)
