...
- Produces:
  ...
  - `hardy.latex.compile` — `TexError(file: str | None, line: int | None, message: str)`; `CompileResult(success: bool, errors: list[TexError], log_tail: str, pdf_path: Path | None)`; `DEFAULT_ENGINE = ["tectonic", "--untrusted", "--chatter", "minimal"]`; `compile_tex(source: str, staging: Path, *, engine: list[str] | None = None, extra_env: dict[str, str] | None = None, timeout: float = 120.0) -> CompileResult` for local engines, and `compile_tex_sandboxed(source: str, staging: Path, *, image: str = "hardy-tex:dev", timeout: float = 120.0) -> CompileResult` for the sandboxed path (lazy-imports `hardy.sandbox.runner` from Task 7 — unit tests here never touch it). The sandboxed path exposes **no writable host mount**: the container gets read-only `/staging` plus its quota'd `/scratch` tmpfs, streams artifacts back as a tar on stdout (capped at 64 MB), the trusted host extracts exactly `main.pdf`/`main.log`, and any host-side abort `docker kill`s the named container. The engine argv gets `"main.tex"` appended — so a docker-prefixed argv slots straight in. Containment: subprocess environment **scrubbed** to `PATH`, `HOME=<staging>`, plus `extra_env`; combined output hard-capped at 1 MB (over-cap kills the compile and reports it); `main.log` read tail-only; stale `main.pdf`/`main.log` deleted before every run. The sandboxed engine compiles in the quota'd `/scratch` tmpfs with the source mounted read-only and copies back only `main.pdf`/`main.log`.
...
Build both (after `scripts/setup_lean.sh` has populated `vendor/repl`). On the very first build, record the tectonic digest and bake it into the Dockerfile's `TECTONIC_SHA256` default:

```bash
curl -fsSL "https://github.com/tectonic-typesetting/tectonic/releases/download/tectonic%400.15.0/tectonic-0.15.0-x86_64-unknown-linux-musl.tar.gz" | sha256sum
# paste the printed digest into the TECTONIC_SHA256 ARG default, commit, then:
docker build -f docker/Dockerfile --target tex  -t hardy-tex:dev  .
docker build -f docker/Dockerfile --target lean -t hardy-lean:dev .
```

Expected: both images build; the `sha256sum -c` line fails the `tex` build on any compiler drift, and the `diff` line fails the `lean` build loudly if the repl checkout drifted from the pinned toolchain. (The unfilled `RECORD-DIGEST-AT-FIRST-BUILD` default fails the build by design — the digest must be recorded, never skipped.)
...
**Interfaces:**
- Consumes: `ReplPool.check_proof` with `spec_factory` (Task 5); `repl_argv`, `sandboxed_worker_spec`, `LEAN_PROJECT` (Tasks 2/7); `render_writeup` (Task 6); `compile_tex`, `compile_tex_sandboxed` (Task 6); the `hardy-lean:dev` and `hardy-tex:dev` images (Task 7).
