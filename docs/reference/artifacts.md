...
| `delegation.recovered` | `reason`, `recovered_at` | Work that was active when the process died; state `unknown`, every usage dimension unknown. Distinct from every other ending. Recovery releases it before the new session's root epoch begins, so it does not count against the new session's budget. |
