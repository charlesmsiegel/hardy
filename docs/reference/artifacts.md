...
Only Lean kernel acceptance justifies `kernel_verified` or `verified_modulo`; TeX compilation checks document construction, never mathematical truth. `sorryAx` is a hole no approval converts into an assumption; a run naming it lands at `partial`, and `verified_modulo` is reachable only where a human approved the assumption, which is the interactive path. `prove` and `batch` run unattended and fail closed instead.
...
`outcome` reads: `solved` is a true entry the run closed with a clean axiom audit; `solved_other` is a false entry the run closed anyway; `unsolved` is a true entry it did not close; `refused` is a false entry it correctly declined; `exhausted` hit a turn or wall-clock limit before either; `graded` is a false entry it neither refused nor solved; `invalid` is a run the recorded-run audit itself could not make sense of.
