...
`transcript.jsonl` is the append-only trace of the conversation that produced `session.json`: one JSON object per line, each carrying `timestamp`, `parent_id`, an `entry_id` derived from the event's own content (so the file forms a hash-linked chain, not just a sequence), and a `type`. A `user` event carries `author: "hardy"` when the turn was started by Hardy itself to carry background results in, so a reader never mistakes it for a line the person typed. A `tool` event carries `detached: true` when the call outlived the turn's grace and was answered with a placeholder naming a background job; the real result then arrives as a `job` event (`job_id`, `name`, `arguments`, the original `call_id`, `status` of `finished` or `cancelled`, `seconds`, and the whole `result`), which is owed to the model until a `job_delivered` event names it in `job_ids`, written once the provider accepted the request that carried it. A `turn` event's `status` is `cancelled` when the user interrupted it directly, or `abandoned` when the session moved on without a reply arriving at all; both matter to a later reader because a turn the user walked away from is otherwise indistinguishable from one they waited for, and only the transcript survives to say which. A `conversation_branch` event records a fork or an abandonment of a leaf, each naming the branch it acted on.
...
| --- | --- | --- |
| `FormalEvidence` | `subject`, `scope`, `context`, `outcome`, `module`, `declaration`, `statement`, `axioms`, `signature`, `source_sha256`, `recorded_at`, `inputs` | What the axiom audit established about one declaration, bound to the exact ledger item, scope and context it was minted for. `outcome` is `kernel_proof`; `axioms` is the complete set Lean reported; `signature` is the module's build signature at the time (the same digest the stored audit verdict carries), `source_sha256` the module source, and `inputs` the module and every workspace module it imports, transitively, each with its source digest, which is what `hardy check` compares with the tree since it has no workspace to recompute the signature. A record minted before `inputs` existed carries none, serializes without the field so its digest is unchanged, and binds only its own module; the next save that verifies the declaration re-mints it with them. Written by `workflows/interactive/evidence.py` when a save is recorded or a change set is admitted, and only for a declaration graded `verified`: a hole or an approved assumption mints nothing. A save that verifies a declaration already accepted re-mints when the build signature or the inputs moved (the module, a module it imports, the shared sources, the toolchain), so the record always describes the build the kernel last checked. |
...
| --- | --- | --- |
| `delegation.created` | `spec`, `parent_id`, `created_at`, `owner` | A node exists; `owner` is the token of the session that created it (absent in journals written before it was recorded). `spec` is the frozen request (objective, exact project refs, scope, lease, concurrency, policies, who asked). The synthetic `root` node carries the session's ceilings. A `delegation.started` whose payload says `interior` marks a container for coordinated children: it runs no worker, and recovery leaves it live rather than marking it unknown. One whose payload says `compute` is a detached computation: a Lean, LaTeX or computer algebra call that outlived the turn's grace, with `task_mode` `compute`, no project refs, no scope, a lease of zeros and no worker slot; it ends `completed` whether its answer was green or red, and its `result.json` holds the tool's whole output. A computation is bounded by its tool's own timeout, not by a lease: its reported seconds are never charged to the root, and `/jobs` shows them apart as the root's computation usage. |
| `budget.reserved` / `budget.released` | `lease`, `slots`, `epoch`, `joined` | A reservation under the parent's allocatable resources, and its return once the node is terminal. The reservation is re-checked against the parent's balance under the journal's own lock as it is recorded, so two processes that both passed the check on one stale balance cannot both record; the loser is journaled cancelled and released. A released node goes on charging its parent what it used; where its checks or seconds are unknown, it is charged its whole lease in them, which is the most it could have spent, or what it had already reported where that was more. The root's reservation also names an `epoch`, the owner token of the session that opened it: the root lease is a budget for one session, so a session that opens when no session of the current epoch is live re-reserves it under an epoch of its own, and children released before that epoch began (the last session's work, interrupted work included) no longer count against it. Children still holding a reservation, or released during the epoch, do. A session that opens while one of the current epoch's sessions is still live (a second terminal, a browser tab, a chat switch) instead records `joined` with its own token and the current `epoch`, and with the epoch's current `lease` and `slots` rather than its own: concurrent sessions on one problem share one budget under the ceiling the epoch currently holds (the first session's, or a survivor's own once it was left alone and re-reserved; a session that joins after that adopts the survivor's), and the epoch ends only once every session in it has. While more than one session of the epoch is live, none of them changes that reservation, up or down; a session left as the epoch's only live one re-reserves to its own ceilings, in the same epoch. The decision and the reservation are made under the journal's lock. A reservation with no `epoch`, as every one written before epochs existed, starts none. |
...
| `delegation.started` / `progress` / `paused` / `resumed` / `waiting` | | Lifecycle. Its payload names the `owner` token of the process that started the work; recovery leaves work alone while that process still holds its token (an OS lock under `delegations/owners/`), so a second process opening the workspace never retires a worker the first still runs. |
...
| `delegation.recovered` | `reason`, `recovered_at` | Work that was active when the process died; state `unknown`, every usage dimension unknown. Distinct from every other ending. When the recovering session opens a new root epoch, recovery releases it before that epoch begins, so it does not count against the new budget. When the session instead joins a live epoch (another session on the problem is still open), it is released during that epoch and charged like any other released node whose usage is unknown: its whole lease in checks and seconds. |
...
| `scheduler.lane_changed` | `lane`, `by` | A delegation moved to another lane (`user_pinned`, `exploit`, `explore`, `verify`, `blocker`). |
| `coordinator.invoked` / `coordinator.plan_proposed` / `coordinator.plan_applied` / `coordinator.plan_invalid` | `view_digest`, `authority` / `plan` / `outcomes` / `answer` | One coordinator checkpoint: the digest of the bounded view it was shown and the authority it held; the structured plan it answered with; what each action came to, applied or refused with reasons; or the answer that was not a plan, in which case the plan was `noop`. |
| `coordinator.human_decision_requested` / `coordinator.synthesis_requested` | `question`, `by` / `by` | An escalation to the human (always a sticky attention item) or a request for a synthesis pass. |
| `admission.requested` | `finding`, `by` | A coordinator asked that a finding be routed to project admission; admission itself is a separate act. |
...
| `context.pushes_consumed` | `findings` | The pushed findings a recipient took into its next context: rendered at the top of its launch prompt if it had not started, otherwise appended to its next tool result, the next provider boundary an active worker has. |
| `workspace.overlay_created` | `generation` | A private copy of the Lean workspace this delegation may write: its generation id, the project revision and workspace digest it was taken from, the parent generation when inherited from a parent's overlay, and the build environment. |
| `workspace.changeset_proposed` | `change_set`, `generation`, `base_project_revision`, `files` | The worker returned a change set (`change_set.json` beside its result): per-file create/modify/delete against base digests. A proposal only; admission re-verifies on the current head. |
| `admission.attempt` | `attempt` | One phase of an authoritative admission: `proposed`, `reconciled`, `files_prepared`, `verification_complete`, `files_committing`, `files_committed`, `ledger_committed`, `completed` or `failed`, each with the project head revision it was taken against. `files_committing` is journaled before the first authoritative write, so a crash mid-mutation is visible; a `files_committing` or `files_committed` phase not followed by `ledger_committed` is an incomplete admission, and only `ledger_committed` makes anything true. |
| `admission.incomplete` | `attempt`, `phase` | Recovery found files committed without a ledger commit; a sticky attention item until a human resolves it. |
...
| `attention.subscribed` | `subscription` | An explicit routing override: owner, source delegation, trigger classes (`terminal`, `finding`, a finding kind, `priority`, `decision`, `progress`), delivery mode (`queue`, `notify`, `interrupt`) and recipient. `interrupt` is reachable only this way; `terminal` and `progress` triggers name the subscribed delegation itself, finding triggers cover its subtree. |
| `attention.interrupt_requested` | `item_id` | A subscribed interrupt reached the session's hook. The session ends the in-flight turn at the runtime boundary (its transcript records a `turn` cancelled with reason `delegation_interrupt:<item-id>`); the item itself reaches the model through the next request's steering block, never mid-request. |
| `continuation.recorded` / `continuation.started` / `continuation.stale` | `continuation` | The main conversation's own next action awaits delegated work: the delegation awaited, the condition (`terminal` or `interrupted`), the conversation epoch and transcript offset it was recorded at, and the text to resume with. It starts only if the epoch is unchanged and no human turn followed the offset; otherwise it is stale and becomes a queued attention item rather than a replay. |
