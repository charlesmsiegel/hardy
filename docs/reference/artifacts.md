...
| `admission.attempt` | `attempt` | One phase of an authoritative admission: `proposed`, `reconciled`, `files_prepared`, `verification_complete`, `files_committed`, `ledger_committed`, `completed` or `failed`, each with the project head revision it was taken against. A phase after `verification_complete` that is not followed by `ledger_committed` is an incomplete admission; only `ledger_committed` makes anything true. |
