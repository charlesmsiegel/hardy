...
Sweeps a committed tactic set over every canonical statement and writes the tier file, which is what says how much of each result automation already closes. Rows are carried forward from the existing tier file rather than re-elaborated, but only when three digests all agree: the environment identity and the procedure digest must match the prior baseline (a Mathlib upgrade, another machine or a change to the sweep code invalidates every row at once), and then each entry is reused only where its own statement digest is unchanged and the prior row still has the shape the entry now needs. That last gate catches a relabelling: the statement digest deliberately excludes `expected`, so a true entry turned into a twin keeps its digest while its old row carries no negation baseline, and it is swept again rather than reused. A corrected statement re-sweeps only that entry. Rows are never restamped: a carried row keeps the statement digest it was measured against, and after the environment or procedure digest moves, no row is carried at all. Exits `1` if the sweep found problems with the corpus, including an entry whose stage A did not run (a Lean error outside every tactic block), which gets no row rather than a tier and is swept again next time. Exits `2` when a selection would carry rows across a moved digest.

| Option | Default | Env var | Meaning |
| --- | --- | --- | --- |
| `--problems` | `corpus` |  | The corpus to sweep. |
| `--out` | `evals/baseline.json` |  | Where the tier file is written. |
| `--only` | the active entries with no usable baseline row |  | Comma-separated entry ids. A named entry is always re-swept, even when its identity has not moved. An unnamed entry keeps its row, with the statement digest it was measured against, only while the tier file's environment and procedure digests match; otherwise the command refuses with exit `2` rather than restamp it. With no `--only`, `--only-file` or `--status`, the sweep covers the active entries with no row, or with a row measured against a statement that has since changed (or a twin's row with no negation sweep); when the tier file's environment or procedure digest has moved, it covers every entry the file holds plus those, and says why on stderr. It refuses with exit `2` when there are none, naming `--only` as the way to force a resweep. |
