...
| `--only` | the active entries not yet run under this condition |  | Comma-separated entry ids. With no `--only`, `--only-file` or `--status`, the run covers the active entries with no valid row on a board `evals pool` would accept under this exact model, mode, limits and environment (what `evals todo` lists as `unevaluated_active`), and refuses with exit `2` when there are none, naming `--only` as the way to force a rerun. An entry holding only some of its repeats is named on stderr and not selected. |
