...
Serves the browser client (the "workbench") on `127.0.0.1` and prints the URL. Unlike `hardy chat`, it never treats the directory it was started in as a root: the page lists and opens only the problems recorded in the user's project registry, `~/.hardy/projects.json` (see [the on-disk layout](on-disk-layout.md#user-level-hardy)), and the registry is written only by the page itself. `New project` creates a problem under `~/.hardy/projects/<name>`, or under a location typed into the form, registers it and opens it; `Add existing` registers a directory that already holds a Hardy record, or every recorded problem under a directory that holds several; `forget` drops an entry and never touches the directory; `Close project` closes the session and leaves nothing open. A `/project new` or `/project switch` typed into the composer registers wherever it lands, so the list stays truthful. With nothing registered, or when the project last opened no longer holds a record, the server starts with no session: the page lands on the project menu, every project-scoped request answers `409` with the sentence `No project is open. Open one from the project menu.`, and nothing is written anywhere until a project is created or added. `HARDY_ROOT` and the config's `root` do not apply to this command. A project made in the browser is opened at the terminal with `hardy chat --root <location> --project <name>`.

The page runs the same session, record and slash-command registry as `hardy chat`, one live session per problem, and adds a list of chats per problem: `main` is the transcript beside the record, and every other chat is its own `chats/<id>/transcript.jsonl`. Opening a chat reopens the problem's session the way `/project switch` does, so it is refused while a turn or a command is running, and it names a registered problem and a chat that already exist rather than creating either; opening the chat that is already open changes nothing. Making a problem from the page is held to `/project new`'s guard: a path already registered, or a directory Hardy did not scaffold, is refused rather than having a record and a `lean/` tree scattered through it. The chat header carries a picker over the rows `/model` offers, the composer lists the commands a typed `/` prefix could be and queues a message sent during a turn, and a Help panel lists every command from that same registry, the prompt shortcuts, the project's own commands and the page's keys.

The page is organized into tabs: Home, Chat, Results, Ledger, Jobs, Runs, Publications, Library, Files and Checkpoints sit in the tab bar, with Status, Environment and Help beside it. Every one of them has a page; a hash that names no route at all says so rather than claiming a page is unbuilt. Ledger draws both a list/item view and its provenance graph, toggled from the page itself. Status shows what `/status` prints and `Spend · per scope`, where only the scopes something actually measures carry figures -- spend is kept as one session-wide total, so the per-branch cards say they are not measured rather than showing a number nobody computed.

Files is an editor. It lists the `lean/`, `tex/`, `cas/` and built-PDF trees with each file's size, mtime and -- for Lean -- the kernel verdict over the declarations it makes, opens what is picked (including the `cas/` cell journal, drawn as cells, with Re-run and a new-cell composer that raise real `/cas` commands for review), and shows files staged for upload. A Lean or TeX file opens in an editor with `Save & check`, which writes through the session's own save: the file is checked, every file that imports it is rebuilt, the result and documentation gates run and the axioms are audited, exactly as they are for a file the model saved. While the buffer differs from what is on disk the saved verdict is not shown against it, because it does not describe that text. Refusals are printed verbatim. Beside the editor: the name under the caret resolved against the installed packages' declarations, with its real source at its real lines; what the ledger says about each declaration the file makes; and a `#check` box that runs against the file's import set and saves nothing. `docs/design/trust-boundary.md` describes the write path. Stop the server with Ctrl+C.

| Option | Default | Env var | Meaning |
| --- | --- | --- | --- |
| `--project` | the project last opened in the browser, if it still holds a record; else nothing is opened |  | A registered project to open first, by name or by path. A name shared by two registered projects is refused with both paths listed; a name that is not registered is refused with the registered names listed, since projects are created and added in the browser. There is no `--root`: the project's own directory says where it is. |
| `--chat` | `main` |  | Which chat of that project to open first; as for `hardy chat`, it must already exist. Needs a project to open, so it is refused when nothing would be. |
| `--port` | an ephemeral port |  | The loopback port to listen on. |
| `--open` | off |  | Open the page in the default browser once the server is up. This is what the installer's launcher passes. |
...
Checks every problem of a root (a subdirectory holding a record or a ledger) against each other, the files on disk and the research-status rules, reading each ledger through Hardy's own store; it never writes a ledger. A recorded problem with no ledger yet is listed and not failed. A ledger that does not read (a sequence gap, a digest mismatch, malformed JSON) is reported as that problem's failure and the other problems are still checked. Problems are checked upstream first, in the order their mirrors dictate. Per problem it enforces that every assessed item carries a status from the vocabulary (`open`, `llm proved`, `human verified`, `lean verified`, and the input statuses `published input`, `external research input`, `imported`); that nothing assessed above `open` depends on an `open` item, following a mirror through as many mirrors as it takes to the item it mirrors; that a `uses` relation targets an external result; that the `depends_on`/`uses` graph is acyclic; that a mirror of another problem's item names a problem of this root checked before it and an item still there with the same digest and status; that a `lean_declaration` semantic names a declaration under the problem's `lean/` or the root's `.hardy/lean/` by its qualified name or a suffix of it; that every artifact an item or a relation references by a path relative to the problem, directly or behind its evidence, still matches its bytes, whether or not the item is assessed (the path must name a file inside the root, a mirror's `../<problem>/ledger/...` included, read through the same guard as every project file, so a path that leaves the root, an absolute path (a Windows drive path such as `C:\notes.md` included, which is not a scheme) or a symlink is refused rather than hashed; an artifact named by a URI with a scheme, such as `arxiv:` or `manuscript:`, belongs to the store that issued it and is not read here; one that does not read is that record's failure); that every Lean source under `lean/` and `.hardy/lean/` reads, a symlink anywhere under either being refused as the workspace refuses it; and that a `lean verified` item has a resolved `prove` obligation on its current revision whose evidence the problem's own `evidence/` journal authenticates and whose formal evidence describes the Lean sources as they stand, the module the kernel checked and every module of the problem it imports still being under `lean/` or `.hardy/lean/` with the digests the evidence recorded, one such obligation sufficing (the shared sources and the toolchain are in the evidence's build signature, which only a save with a workspace recomputes; the check binds what it can read). `human verified` is refused, since no evidence mechanism backs it yet. Problems whose mirrors form a cycle are still checked, after the ones that could be ordered, and a mirror of a problem not yet checked is reported as such beside the cycle. Relations are read at their current endpoints: the check is of the board as it stands, so a relation pinning an older revision of an item is compared against that item's current status. No status certifies truth; the check enforces the rules between statuses. Prints the problem order, one board line per problem, every failure, and a verdict. Exits `1` when any check failed.

| Option | Default | Env var | Meaning |
| --- | --- | --- | --- |
| `--root` | the configured root, else the current directory | `HARDY_ROOT` | The root whose problems to check. |
| `--mermaid` | off |  | Also print each problem's `depends_on`/`uses` graph as a Mermaid flowchart, prerequisite to dependent, coloured by status, with positional node identifiers so ids that differ only in punctuation stay distinct. |
...
Sweeps a committed tactic set over every canonical statement and writes the tier file, which is what says how much of each result automation already closes. Rows are carried forward from the existing tier file rather than re-elaborated, but only when three digests all agree: the environment identity and the procedure digest must match the prior baseline (a Mathlib upgrade, another machine or a change to the sweep code invalidates every row at once), and then each entry is reused only where its own statement digest is unchanged and the prior row still has the shape the entry now needs. That last gate catches a relabelling: the statement digest deliberately excludes `expected`, so a true entry turned into a twin keeps its digest while its old row carries no negation baseline, and it is swept again rather than reused. A corrected statement re-sweeps only that entry. Rows are never restamped: a carried row keeps the statement digest it was measured against, and after the environment or procedure digest moves, no row is carried at all. Exits `1` if the sweep found problems with the corpus, including an entry whose stage A did not run (a Lean error outside every tactic block), which gets no row rather than a tier; the next bare `evals baseline` sweeps it again if it is active, and a non-active one must be named. Exits `2`, before any elaboration, when a selection would carry rows across a moved digest. Back up the tier file before a re-sweep after such a move: the first checkpoint replaces it.

| Option | Default | Env var | Meaning |
| --- | --- | --- | --- |
| `--problems` | `corpus` |  | The corpus to sweep. |
| `--out` | `evals/baseline.json` |  | Where the tier file is written. |
| `--only` | the active entries with no usable baseline row |  | Comma-separated entry ids. A named entry is always re-swept, even when its identity has not moved. An unnamed entry keeps its row, with the statement digest it was measured against, only while the tier file's environment and procedure digests match; otherwise the command refuses with exit `2` rather than restamp it. With no `--only`, `--only-file` or `--status`, the sweep covers the active entries with no row, with a row that records no statement digest, with a row measured against a statement that has since changed, or (a twin) with a row that has no negation sweep, plus any entry, active or not, whose row has an attempt recorded `not_run`; when the tier file's environment or procedure digest has moved, it covers every entry the file holds plus those, and says why on stderr. It refuses with exit `2` when there are none, naming `--only` as the way to force a resweep. |
| `--only-file` | as `--only` |  | A file of entry ids, one per line; `-` reads stdin. Named entries behave as with `--only`. |
| `--status` | unset |  | Select by corpus status, e.g. `--status active`. Repeatable. Given alone it selects, and so re-sweeps, every entry at that status, including those already swept; given with `--only` it narrows the named set. |
...
| `--only` | the active entries not yet run under this condition |  | Comma-separated entry ids. With no `--only`, `--only-file` or `--status`, the run covers the active entries with no valid row on a board `evals pool` would accept under this exact model, mode, limits and environment (what `evals todo` lists as `unevaluated_active`), and refuses with exit `2` when there are none, naming `--only` as the way to force a rerun. An entry holding only some of its repeats is named on stderr and not selected, and so is an entry held by boards `evals pool` refuses together (`conflicted_active`): rerunning it would claim a slot one of them still holds, so the remedy is to set a board aside. When those are all that is left, the run refuses with exit `2` and says so. |
...
Browse the corpus in a local page that re-reads from disk on every refresh: statement, Lean, and classification side by side, with the objections `evals corpus check` would raise shown against the entries that earned them, so a shard edited in an editor shows up, correct or broken, on the next reload. `--host` defaults to `127.0.0.1` -- loopback only -- because a working corpus is not a published site; `--host ::` binds every interface over IPv6 the same way an explicit `--host 0.0.0.0` does over IPv4. Every request must carry a `Host` this process recognizes as its own, compared case-insensitively since a `Host` is a DNS name and a client may spell it in any case, and (for an IP literal) by the address it names rather than necessarily the spelling `--host` was given, since a browser sends its own canonical spelling regardless: for a specific `--host` (the default included), that host itself, plus `localhost`/`127.0.0.1`/`::1` when the host given is one of those; for the wildcard binds `--host 0.0.0.0` or `::`, the machine's own hostname and FQDN, every address either resolves to, and loopback -- never an arbitrary name, so `--host 0.0.0.0` widens who can *reach* the port, not who a request may claim to come from. On `--port 80`, the portless form of every admitted host is accepted too, since a client reaching the default HTTP port sends a bare `Host: <host>` with no port on it. A LAN client whose address is not among those -- reached by a bare IP no reverse-DNS lookup names -- gets a 403 from that Host check alone; serve it with `--host <that address>` instead. The URL the command prints is always one of these admitted hosts -- loopback for a wildcard bind, since a wildcard has no address of its own. Past `Host`, a write additionally needs an `Origin` that matches it and an `X-Hardy-Token` header echoing the token stamped into the page it served, so no page from elsewhere -- another site open in the same browser, or another host on the network -- can drive the one write route, `POST /api/review`, even if it can reach the corpus to read: that route records a human's faithfulness verdict, with the digests computed server-side so a stale tab cannot approve a statement that has since changed. Entries themselves are still authored in a text editor and checked with `hardy evals corpus check`.
...
Reports what is left to sweep or run under the pooling key this checkout would produce, as JSON on stdout. Takes the same budget and repeat flags as `evals run` (`--mode`, `--strategy`, `--history-mode`, `--max-turns`, `--wall-seconds`, `--repeats`), with the same types and defaults, because all of them feed the same pooling key: without them `todo` would silently report the key of a default run while `evals run --max-turns 40` recorded a different one, and `evals pool` would then refuse the board it had just been told to produce. It counts only what `evals pool` would accept together: `boards_counted` lists the boards under the key that pass their own audit (the row re-derivation and every exposure journal included) and share no `(id, repeat)` slot with another such board, `boards_refused` maps each board that matches the key but fails that audit to its findings (and also a board whose exposure record fails the board-level check `evals pool` does not run, its findings prefixed `board-level exposure check:`, or one that cannot be read again), `boards_conflicting` maps each board that passes it but claims a slot another admitted board also claims (whatever the row's outcome) to the slots and the other boards, since `evals pool` refuses those together, `unevaluated_active` lists the active entries with no valid (non-`invalid`) row on a counted board, `partially_evaluated_active` lists those holding some of their repeats but not all, and `conflicted_active` lists those a conflicting board holds a slot of. Neither of the last two is selected by a default `evals run`; for a conflict, stderr names the boards and the remedy, which is to move all but one of each conflicting set out of the scoreboards directory, not to rerun. `baseline_sweeps` lists exactly what a bare `evals baseline` would sweep now (the `--only` default above, every held row included after an environment or procedure digest move), `baseline_moved` says why no row can be carried after such a move (otherwise `null`), and `unbaselined_active` is the active part of `baseline_sweeps`.
...
Replace the model placeholder with the configured provider identity. A shared journal reserves estimated input tokens plus the actual output cap before each call, then settles against reported usage. A call whose usage is missing or incomplete, including one that timed out, lost its connection, or failed with a server error, stays charged at its reservation, or at the counters it did report with the output cap standing in for a missing output count, whichever is more, as does a reservation nobody settled; later calls are still admitted while the remaining headroom covers them. Only a 429 or 529 rejection, which the provider answers before doing any work, settles at zero. A reported overrun prevents later calls, including one where the counters a partial report did state, or their derived cost, already exceed the reservation. The budget summary a run records (`provider_budget` in its provenance) counts only complete reports in `reservation_overruns`, so a partial report that overran can leave it at `0` while its `ending_limit` reads `reservation_overrun`. The estimate is not a hard token ceiling and not a provider invoice cap. An optional cost limit requires an explicit tariff in the policy. Policy changes cannot rewrite an existing budget journal. The configured policy meters every session the process opens, including `/project switch`, `/project new`, `/checkpoint restore` and browser opens, so a problem whose journal was started under a different policy refuses to open until the configured policy matches it again. `HARDY_PROVIDER_BUDGET` can select the policy path instead of the config key.
