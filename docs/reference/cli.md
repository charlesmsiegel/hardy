...
Browse the corpus in a local page that re-reads from disk on every refresh: statement, Lean, and classification side by side, with the objections `evals corpus check` would raise shown against the entries that earned them, so a shard edited in an editor shows up, correct or broken, on the next reload. It binds `127.0.0.1` by default because a working corpus is not a published site. Every request must carry a `Host` this process recognizes as its own: for a specific `--host`, that host itself, plus `localhost`/`127.0.0.1`/`::1` when the host given is one of those; for the wildcard binds `--host 0.0.0.0` or `::`, the machine's own hostname and FQDN, every address either resolves to, and loopback -- never an arbitrary name, so `--host 0.0.0.0` widens who can *reach* the port, not who a request may claim to come from. A LAN client whose address is not among those -- reached by a bare IP no reverse-DNS lookup names -- gets a 403 from that Host check alone; serve it with `--host <that address>` instead. Past `Host`, a write additionally needs an `Origin` that matches it and an `X-Hardy-Token` header echoing the token stamped into the page it served, so no page from elsewhere -- another site open in the same browser, or another host on the network -- can drive the one write route, `POST /api/review`, even if it can reach the corpus to read: that route records a human's faithfulness verdict, with the digests computed server-side so a stale tab cannot approve a statement that has since changed. Entries themselves are still authored in a text editor and checked with `hardy evals corpus check`.
