...
Browse the corpus in a local page that re-reads from disk on every refresh: statement, Lean, and classification side by side, with the objections `evals corpus check` would raise shown against the entries that earned them, so a shard edited in an editor shows up, correct or broken, on the next reload. It binds `127.0.0.1` by default because a working corpus is not a published site. Every request must carry a `Host` this process actually answers to (its bind address, or a loopback alias) and, unless it is a plain navigation, an `Origin` that matches it -- so `--host 0.0.0.0` widens who can *reach* the port, not who a request may claim to come from. It is a viewer with one write route: `POST /api/review` records a human's faithfulness verdict, with the digests computed server-side so a stale tab cannot approve a statement that has since changed; that route additionally requires an `X-Hardy-Token` header echoing the token stamped into the page it served, so no page from elsewhere -- another site open in the same browser, or another host on the network -- can drive it even if it can reach the corpus to read. Entries themselves are still authored in a text editor and checked with `hardy evals corpus check`.
