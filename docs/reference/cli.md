...
The page is organized into tabs: Home, Chat, Results, Ledger, Jobs, Runs, Publications, Library, Files and Checkpoints sit in the tab bar, with Status, Environment and Help beside it. Five of those -- Results, Runs, Publications, Checkpoints and Status -- are visible and clickable but have no page behind them yet; clicking one renders an honest "not built yet" state rather than a blank panel or a disabled tab. Ledger currently draws only the ledger's provenance graph, not the list view. Files is read-only: it lists the `lean/`, `tex/`, `cas/` and built-PDF trees, opens what is picked (including the `cas/` cell journal, drawn as cells), and shows files staged for upload -- but there is no file-write endpoint on the server at all, so editing, `Save & check`, diagnostics, goals at cursor, declaration lookup and the Mathlib trace are not yet built. Stop the server with Ctrl+C.
