...
The page is organized into tabs: Home, Chat, Results, Ledger, Jobs, Runs, Publications, Library, Files and Checkpoints sit in the tab bar, with Status, Environment and Help beside it. Of those, only Status is visible and clickable with no page behind it yet; clicking it renders an honest "not built yet" state rather than a blank panel or a disabled tab. Ledger draws both a list/item view and its provenance graph, toggled from the page itself. Files is read-only: it lists the `lean/`, `tex/`, `cas/` and built-PDF trees, opens what is picked (including the `cas/` cell journal, drawn as cells), and shows files staged for upload -- but there is no file-write endpoint on the server at all, so editing, `Save & check`, diagnostics, goals at cursor, declaration lookup and the Mathlib trace are not yet built. Stop the server with Ctrl+C.
