...
model received until a model receipt says so. Sticky items outlive receipts
and leave the pending view only when handled. None of this is mathematical
truth; it is what needs looking at.
"""
...
class AttentionInbox:
    ...
    def derive(self, event: DelegationEvent, tree: DelegationTree) -> AttentionItem | None:
        ...
        if event.kind in _TERMINAL_KINDS:
            ...
            summary = f"{delegation.id} ({delegation.spec.objective}) {status}"
            if tail:
                summary += f": {tail}"
            return AttentionItem(
                id=item_id, delegation_id=delegation.id, source_event=event.sequence,
    ...
    def render_for_model(items: tuple[AttentionItem, ...], *, budget_items: int = 5) -> str:
        ...
        for item in shown:
            ...
            lines.append(f"- {item.summary}{flag} (delegation {item.delegation_id}; details: {', '.join(item.detail_refs)})")
